...
function t8() {
	...
	let out = b8.filter(function (element, index) {
...
function t9() {
	...
	let out = b9.filter(elem => {
		if ( typeof(elem) == 'number') b9_num.push(elem);
		if (typeof(elem) == 'string') b9_string.push(elem);
	})
	console.log(b9_num);
...
function t10() {
	...
	b10.filter(function (item, index) {
		if (!item.includes(3)) b10.splice(index, 1)
	});
	b10_res = b10;
