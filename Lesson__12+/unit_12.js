...
function f3() {
	...
	out.innerHTML = a3[a3.length-1][1][1];
	return a3[a3.length-1][1][1];
}
...
function f10() {
	...
	for (let i = 0; i < a10.length; i++){
		for (let r = 0; r < a10[i].length; r++){
			if(typeof a10[i][r] == 'string') {
				out += a10[i][r] + ' ';
			}
		}
	}
...
function f11() {
	...
	for (let i = 0; i < a11.length; i++){
		for (let r = a11[i].length - 1; r >= 0; r--){
			out += a11[i][r] +' ';
		}
	}
