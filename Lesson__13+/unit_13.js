...
function f6() {
	let inpOne = document.querySelector('.i-61').value;
	let inpTwo = document.querySelector('.i-62').value;
	let m = '';
		for( let key in a6)	{
			a6[inpOne] = inpTwo;
		m += key + ':' + a6[key] + '<br>'; 
	}
	document.querySelector('.out-6').innerHTML += m;
}
...
function f19() {
	...
	let m = '';
	for ( let key in a19){
		for ( let keyOne in a19[key]){
			if ( a19[key][keyOne].toLowerCase() == inp.toLowerCase()){
				m = key;
			}
		}
	}
